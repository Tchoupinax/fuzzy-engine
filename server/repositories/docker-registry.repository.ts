...
export class DockerApiRepository implements RegistryApiRepository {
  ...
  async listRepositories(
    ...
    const repositoryNames: Array<DockerRegistryRepositoryName> =
      await this.internalListRepositories(limit, offset, name);

    const repositories = await Promise.all(
      repositoryNames.map((repositoryName: string) => {
        return this.internalListRepositoryTags(repositoryName).then(
          ({ name, tags }) => {
            return {
              name,
  ...
  async listRepositoriesTags(
    ...
    const tagsWithDigest = await Promise.all(
      tags.map(async (tag) => {
        const { layers, digest } = await this.internalGetManifestData(
          repositoryName,
          tag,
        );
  ...
  private getArchitectures(
    ...
    return axios({
      ...
      .catch((err) => {
        logger.error(
          { error: err.response.data, repositoryName, tag },
          "getArchitectures",
        );
        return [];
      });
  ...
  private async internalListRepositories(
    ...
    if (name && name.isSome()) {
      return answer.data.repositories.filter((repositoryName: string) =>
        repositoryName.includes(name.get()),
      );
    } else {
  ...
  private getCreationDate(repositoryName: string, tag: string): Promise<Date> {
    return axios({
      method: "GET",
      url: `${this.getComputedUrl()}/v2/${repositoryName}/manifests/${tag}`,
    })
