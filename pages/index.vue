<template>
  ...
  >
    <div>
      <h1 class="text-center title">fuzzy-engine</h1>

      <h2 class="text-center subtitle">Docker registry UI made with love</h2>
      ...
      <div class="flex justify-center">
        <div class="flex justify-center mt-8 w-2/3">
          <form class="w-full" @submit="openList">
            ...
            >
              ...
              <div class="flex">
                ...
                <label
                  for="passwordless"
                  class="flex w-full italic font-bold cursor-pointer"
                >
            ...
            >
              ...
              >
                <input
                  ...
                  @change="saveData"
                />
                Use local AWS authentication <br />({{
                  awsEcr.localAuthentication
                }})
              </label>
            ...
            <div
              v-if="provider === 'scaleway-registry'"
              class="flex flex-col items-center justify-center w-full p-4"
            >
              <input
                v-model="scalewayRegistry.url"
                type="text"
                class="w-full p-1 px-2 mb-4 text-xl font-bold border rounded text-theme-default border-theme-default placeholder-theme-lighter"
                placeholder="url"
                @keyup="saveData"
              ...
              <div class="flex justify-center w-full mb-4">
                <input
                  v-model="scalewayRegistry.token"
                  class="w-full p-1 px-2 text-xl font-bold border rounded-l text-theme-default border-theme-default docker-pull placeholder-theme-lighter"
                  :type="revealed ? 'text' : 'password'"
...
export default {
    ...
    connected() {
      return match(this.provider)
        ...
        .with(
          "github-ecr",
          () =>
            this.githubRegistry.token.length > 0 &&
            this.githubRegistry.nickname.length > 0,
        )
        .with(
          "dockerhub",
          () =>
            this.dockerhub.username.length > 0 &&
        ...
        .with(
          "scaleway-registry",
          () => this.scalewayRegistry.token.length > 0 &&
          this.scalewayRegistry.url.length > 0
        )
        .exhaustive();
    },
  },
  mounted() {
    ...
    this.provider = (getCookie("fuzzy-engine-provider") ??
      "docker-registry-v2") as Provider;
  },
  methods: {
    saveData() {
    ...
    async checkAwsLocalAuthentication() {
      const data = await $fetch(
        `${new URL((window as unknown as any).location).origin}/api/authentication/aws-local`,
      );
